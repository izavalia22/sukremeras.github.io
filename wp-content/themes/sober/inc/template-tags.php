...
if ( ! function_exists( 'sober_has_page_header' ) ) :
	...
	function sober_has_page_header() {
		...
		return sober_get_option( 'page_header_enable' );
	}
endif;
	...
	function sober_get_page_header_image() {
		...
		if ( function_exists( 'is_woocommerce' ) && is_woocommerce() ) {
			...
			if ( is_product_taxonomy() ) {
				...
				$shop_image = get_post_meta( get_the_ID(), 'page_header_bg', true );
			...
			<?php if ( has_post_thumbnail( $post_id ) ) : ?>
				<li>
					<a target="_blank" class="share-pinterest social"
					   href="http://pinterest.com/pin/create/button/?url=<?php echo urlencode( get_the_post_thumbnail_url( $post_id ) ); ?>&description=<?php echo urlencode( get_the_title( $post_id ) ); ?>"><i
				...
				case 'cart':
					...
					printf(
						'<li class="menu-item menu-item-cart">
							<a href="%s" class="cart-contents" data-toggle="modal" data-target="cart-modal" data-tab="cart">
								%s
					...
					if ( defined( 'YITH_WCWL' ) ) {
						printf(
							'<li class="menu-item menu-item-wishlist">
								<a href="%s" class="wishlist-contents" data-toggle="modal" data-target="cart-modal" data-tab="wishlist">
						...
						printf(
							'<li class="menu-item menu-item-wishlist"><a href="%s" class="wishlist-contents" data-toggle="modal" data-target="cart-modal" data-tab="wishlist"><svg viewBox="0 0 20 20"><use xlink:href="#heart-wishlist-like"></use></svg><span class="count wishlist-counter">%s</span></a></li>',
		...
		foreach ( $settings as $setting => $selector ) {
			...
			foreach ( $properties as $key => $property ) {
				if ( isset( $typography[ $key ] ) && ! empty( $typography[ $key ] ) ) {
					...
					$value = 'font-family' == $key ? '"' . $typography[ $key ] . '"' : $typography[ $key ];
...
if ( ! function_exists( 'sober_scrape_instagram' ) ) :
	 ...
	 */
	function sober_scrape_instagram( $username ) {
		...
		if ( false === ( $instagram = get_transient( $transient_key ) ) ) {
			...
			if ( isset( $data['user']['media']['nodes'] ) ) {
				...
				return new WP_Error( 'bad_json_2', esc_html__( 'Instagram has returned invalid data.', 'sober' ) );
			}
