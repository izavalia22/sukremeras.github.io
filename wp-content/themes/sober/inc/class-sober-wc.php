...
class Sober_WooCommerce {
	...
	public function hooks() {
		...
		add_filter( 'wc_add_to_cart_message', array( $this, 'add_to_cart_message' ) );
	...
	public function product_ribbons() {
		...
		if ( $product->is_on_sale() ) {
			...
			} elseif ( $product->get_type() == 'simple' && $product->get_regular_price() != 0 ) {
		...
		<?php if ( 'default' != $style ) : ?>
			...
			<div class="buttons-icon">
				...
				if ( 'quickview' != $style && $quickview && $quickview_behavior == 'view_button' ) {
					printf( '<a href="%s" class="quick_view_button button"><svg width="20" height="29"><use xlink:href="#quickview-eye"></use></svg></a>', esc_url( get_post_permalink() ) );
				}
				?>
			</div>
