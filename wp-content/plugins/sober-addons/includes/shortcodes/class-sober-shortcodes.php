...
class Sober_Shortcodes {
	...
	public static function banner_grid_4( $atts, $content ) {
		...
		return '<div class="sober-banner-grid-4 ' . esc_attr( $atts['el_class'] ) . '">' . $content . '</div>';
	}
	...
	public static function banner_grid_5( $atts, $content ) {
		...
		return '<div class="sober-banner-grid-5 ' . esc_attr( $atts['el_class'] ) . '">' . $content . '</div>';
	}
	...
	public static function banner_grid_6( $atts, $content ) {
		...
		return '<div class="sober-banner-grid-6 ' . esc_attr( implode( ' ', $css_class ) ) . '">' . $content . '</div>';
	}
	 ...
	 * @return array
	 */
	public static function get_coordinates( $address, $refresh = false ) {
		...
		if ( $refresh || $coordinates === false ) {
			...
			if ( $response['response']['code'] == 200 ) {
				...
				if ( $data->status === 'OK' ) {
					...
					$results['error'] = esc_html__( 'Something went wrong while retrieving your map, please ensure you have entered the short code correctly.', 'sober' );
				}
			} else {
